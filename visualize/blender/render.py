...
def render(npydata, frames_folder, *, mode, gt=False,
           ...
           accelerator='gpu',device=[0], pre_idx=None, ours=True):
    if init:
    ...
    data = Meshes(npydata, ours=ours, mode=mode,
    ...
    frameidx = get_frameidx(mode=mode, nframes=nframes,
                            ...
                            frames_to_keep=num, pre_idx=pre_idx)
    ...
    for index, frameidx in enumerate(frameidx):
        if mode == "sequence":
            ...
            mat = data.get_sequence_mat(frac, ours)
        else:
            mat = data.mat
