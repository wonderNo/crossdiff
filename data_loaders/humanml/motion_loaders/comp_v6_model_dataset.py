...
from utils.load_utils import load_and_freeze_clip, encode_text
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dataloader)):
                ...
                tokens = [t.split('_') for t in batch['tokens']]
                ...
                for t in range(repeat_times):
                    ...
                    if t == 0:
                        sub_dicts = [{'motion': sample[bs_i].squeeze().permute(1,0).cpu().numpy(),
                                    'length': batch['lengths'][bs_i].cpu().numpy(),
                                    'caption': batch['text'][bs_i],
                                    'tokens': tokens[bs_i],
