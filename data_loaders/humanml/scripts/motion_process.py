...
class NormExpression3d():
    def __init__(self, device):
        ...
        example_data = np.load('/apdcephfs_cq3/share_1290939/zepingren/humanml3d/joints/000021.npy')
        example_data = example_data.reshape(len(example_data), -1, 3)
        example_data = torch.from_numpy(example_data)
        tgt_skel = Skeleton(self.n_raw_offsets, self.kinematic_chain, device)
        self.tgt_offsets = tgt_skel.get_offsets_joints(example_data[0])
        self.tgt_leg_len = torch.abs(self.tgt_offsets[5]).max() + torch.abs(self.tgt_offsets[8]).max()

    def renorm(self, motion):
        '''
        motion: [B,196,263]
        '''
        B, T, _ = motion.shape
        positions = recover_from_ric(motion) #[B, T, 22, 3]

        src_skel = Skeleton(self.n_raw_offsets, self.kinematic_chain, self.device)
        src_offset = src_skel.get_offsets_joints_batch(positions[:,0])

        src_leg_len = torch.abs(src_offset[:,5]).max(dim=-1)[0] + torch.abs(src_offset[:,8]).max(dim=-1)[0]
        scale_rt = self.tgt_leg_len / src_leg_len
        src_root_pos = positions[:,:, 0]
        tgt_root_pos = src_root_pos * scale_rt.unsqueeze(1).unsqueeze(1)
        quat_params, root_quat_init = src_skel.inverse_kinematics(positions)

        src_skel.set_offset(self.tgt_offsets)
        positions = src_skel.forward_kinematics_batch(quat_params, tgt_root_pos) #[B, T, 22, 3]

        floor_height = positions.min(dim=1, keepdim=True)[0].min(dim=2, keepdim=True)[0][:,:,:,1]
        positions[:, :, :, 1] -= floor_height

        root_pos_init = positions[:,0]
        root_pose_init_xz = root_pos_init[:,0] * torch.tensor([[1, 0, 1]], dtype=torch.float32, device=motion.device)
        positions = positions - root_pose_init_xz[:,None,None]
        positions = qrot(root_quat_init[:,None,None].expand(B, T, 22, -1), positions)
        quat_params[:,1:] = qmul(qinv(root_quat_init[:,None,None].expand(B, T-1, 22, -1)), quat_params[:,1:].contiguous())
        ...
        feet_l = ((feet_l_x + feet_l_y + feet_l_z) < 0.002).float()
        ...
        feet_r = (((feet_r_x + feet_r_y + feet_r_z) < 0.002)).float()
        ...
        positions = qrot(r_rot[:,:,None].expand(B, T, 22, -1), positions)
        root_y = positions[:,:-1, 0, 1:2]

        r_velocity = torch.arcsin(r_velocity[:,:, 2:3])
        l_velocity = velocity[:,:, [0, 2]]
        rot_data = cont_6d_params[:,:-1, 1:].reshape(B, T-1, -1)
        ric_data = positions[:,:-1, 1:].reshape(B, T-1, -1)
        local_vel = qrot(r_rot[:, :-1, None].expand(B, T-1, 22, -1),
                        global_positions[:,1:] - global_positions[:,:-1])
        local_vel = local_vel.reshape(B, T-1, -1)

        new_motion = torch.cat([r_velocity, l_velocity, root_y, ric_data, rot_data, local_vel, feet_l, feet_r],
                         dim=-1)
        new_motion = torch.cat([new_motion, torch.zeros((B, 1, 263), dtype=torch.float32, device=motion.device)],
                     dim=1)
        return new_motion
...
def recover_from_ric2d(data):
    root = data[...,:2].unsqueeze(-2)
    # root = torch.cumsum(root_v, dim=-1)
    positions = data[...,2:44]
    positions = positions.view(positions.shape[:-1] + (-1, 2))
    positions = positions + root
    positions = torch.cat([root, positions], dim=-2)
...
def recover_from_ric_v2(data, joints_num):
    r_rot_quat, r_pos = recover_root_rot_pos(data)
    positions = data[..., 4:(joints_num - 1) * 3 + 4]
    positions = positions.view(positions.shape[:-1] + (-1, 3))
...
def get_matrix_from_vec(vec):
    # (B,T,2)
    vec = vec / torch.sqrt(torch.sum(vec ** 2, dim=-1, keepdims=True))
    mat = torch.stack([vec[:,:, 0], -vec[:,:,1], vec[:,:,1], vec[:,:,0]], dim=2).view((vec.shape[0], vec.shape[1],2,2))
    return mat

def complicate2d(positions, feet_thre=0.002):
    # (B, seq_len, joints_num, 2)
    ...
    feet_l_x = (positions[:,1:, [7, 10], 0] - positions[:,:-1, [7, 10], 0]) ** 2
    feet_l_y = (positions[:,1:, [7, 10], 1] - positions[:,:-1, [7, 10], 1]) ** 2
    feet_l = ((feet_l_x + feet_l_y) < feet_thre).float()

    feet_r_x = (positions[:,1:, [8, 11], 0] - positions[:,:-1, [8, 11], 0]) ** 2
    feet_r_y = (positions[:,1:, [8, 11], 1] - positions[:,:-1, [8, 11], 1]) ** 2
    feet_r = (((feet_r_x + feet_r_y) < feet_thre)).float()

    R0 = torch.eye(2, dtype=torch.float32, device=positions.device)[None,None].expand(B,T,2,2)
    rot_params = torch.zeros((B, T, 21, 2, 2), dtype=torch.float32, device=positions.device)
    for chain in kinematic_chain:
        ...
        for j in range(len(chain) - 1):
            ...
            R_loc = torch.einsum('btij,btjk->btik', R.permute(0,1,3,2), rot_mat)
    ...
    positions = positions[:,:,1:].reshape(B, T, -1)

    rot_data = cont_2d_params.reshape(B, T, -1)

    local_vel = global_positions[:,1:] - global_positions[:,:-1]
    local_vel = local_vel.view(B, T-1, -1)

    # root_v:2, loc_position:21*2, rot_2d:21*2, local_vel:22*2, feet_contact:4--[B,134]
    data = torch.cat([root_v, positions[:,:-1], rot_data[:,:-1], local_vel, feet_l, feet_r], dim=-1)
    data = torch.cat([data, torch.zeros((B,1,134), dtype=torch.float32, device=positions.device)], dim=1)
