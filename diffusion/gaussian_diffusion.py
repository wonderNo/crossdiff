...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if C == 263 or C == 251:
            ...
            model_output = model_output['m']
        else:
            model_output = model.model(x, self._scale_timesteps(t), model_kwargs,
                                       return_m=False, return_j=True)
            model_output = model_output['j']

        if 'inpainting_mask' in model_kwargs.keys() and 'inpainted_motion' in model_kwargs.keys():
    ...
    def training_losses(self, model, batch, t, noise=None, dataset=None):
        """
        ...
        model_output = model(x_t, t, y=batch['enc_text'])
        ...
        if 'valid' in batch.keys():
            mask = batch['valid'].unsqueeze(1).unsqueeze(1).unsqueeze(1) * mask
